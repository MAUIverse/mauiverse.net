...
const OUTPUT_PATH = resolve(process.cwd(), 'src/data/maui-contributors.generated.ts');

const token =
  process.env.MAUI_CONTRIBUTORS_GITHUB_TOKEN ?? process.env.GITHUB_TOKEN ?? process.env.GH_TOKEN;
const strictMode = /^(1|true|yes)$/i.test(process.env.MAUI_CONTRIBUTOR_SYNC_STRICT ?? '');
const forceRefresh = /^(1|true|yes)$/i.test(process.env.MAUI_CONTRIBUTOR_SYNC_FORCE_REFRESH ?? '');
...
function buildOutput(usernames) {
  ...
  return `// This file is generated by scripts/fetch-maui-contributors.mjs.\n// Do not edit manually.\n\nexport const mauiContributorSource = ${JSON.stringify(source)};\nexport const mauiContributorFetchedAt = ${JSON.stringify(fetchedAt)};\nexport const mauiContributorUsernames = ${valuesLiteral} as const;\n`;
}

function createHeaders() {
  const headers = {
    Accept: 'application/vnd.github+json',
    'User-Agent': 'mauiverse.net contributor sync',
...
async function run() {
  if (!token) {
    console.warn(
      'No GitHub token found in MAUI_CONTRIBUTORS_GITHUB_TOKEN, GITHUB_TOKEN, or GH_TOKEN; continuing unauthenticated (lower rate limit).'
    );
  }
