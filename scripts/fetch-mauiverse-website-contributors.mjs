...
const OWNER = 'MAUIverse';
const REPO = 'mauiverse.net';
const PER_PAGE = 100;
const API_BASE = `https://api.github.com/repos/${OWNER}/${REPO}/contributors`;
const OUTPUT_PATH = resolve(
  process.cwd(),
  'src/data/mauiverse-website-contributors.generated.ts'
);

const token =
  process.env.MAUIVERSE_WEBSITE_CONTRIBUTORS_GITHUB_TOKEN ??
  process.env.MAUI_CONTRIBUTORS_GITHUB_TOKEN ??
  process.env.GITHUB_TOKEN ??
  process.env.GH_TOKEN;
const strictMode = /^(1|true|yes)$/i.test(
  process.env.MAUIVERSE_WEBSITE_CONTRIBUTOR_SYNC_STRICT ?? ''
);
const forceRefresh = /^(1|true|yes)$/i.test(
  process.env.MAUIVERSE_WEBSITE_CONTRIBUTOR_SYNC_FORCE_REFRESH ?? ''
);
...
function buildOutput(usernames) {
  ...
  return `// This file is generated by scripts/fetch-mauiverse-website-contributors.mjs.\n// Do not edit manually.\n\nexport const mauiverseWebsiteContributorSource = ${JSON.stringify(source)};\nexport const mauiverseWebsiteContributorFetchedAt = ${JSON.stringify(fetchedAt)};\nexport const mauiverseWebsiteContributorUsernames = ${valuesLiteral} as const;\n`;
}

function createHeaders() {
  const headers = {
    Accept: 'application/vnd.github+json',
    'User-Agent': 'mauiverse.net website contributor sync',
...
async function run() {
  if (!token) {
    console.warn(
      'No GitHub token found in MAUIVERSE_WEBSITE_CONTRIBUTORS_GITHUB_TOKEN, MAUI_CONTRIBUTORS_GITHUB_TOKEN, GITHUB_TOKEN, or GH_TOKEN; continuing unauthenticated (lower rate limit).'
    );
  }
