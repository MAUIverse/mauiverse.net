...
const CONTENT_DIR = resolve(__dirname, '../src/content/toolkit-release');
...
async function loadAuthorMap() {
  const files = (await readdir(CONTRIBUTORS_DIR)).filter(
...
function createHeaders() {
  const headers = {
    ...
    'User-Agent': 'mauiverse.net toolkit release sync',
...
async function fetchReleasesPage(page) {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch toolkit releases page ${page}: ${res.status} ${res.statusText}`);
  }
...
function transformBodyLinks(body, authorKeyToDisplayName) {
  ...
  return body.replace(AT_MENTION_REGEX, (match, username) => {
    ...
    return `[@${username}](/repo-contributors/community-toolkit/?q=${encodeURIComponent(username)})`;
  });
}
...
async function run() {
  if (!token) {
    console.warn(
      'No GitHub token in MAUI_TOOLKIT_CONTRIBUTORS_GITHUB_TOKEN, MAUI_CONTRIBUTORS_GITHUB_TOKEN, GITHUB_TOKEN, or GH_TOKEN; continuing unauthenticated (lower rate limit).'
    );
  }
  ...
  console.log(`Fetched ${releases.length} toolkit releases.`);
  ...
  for (const release of releases) {
    ...
    const name =
      release.name != null && String(release.name).trim() !== ''
        ? String(release.name).trim()
        : tag;
    const body = release.body ?? '';
    const htmlUrl =
      release.html_url ?? `https://github.com/${OWNER}/${REPO}/releases/tag/${tag}`;
  ...
  console.log(`Wrote ${releases.length} toolkit release files to ${CONTENT_DIR}.`);
}
